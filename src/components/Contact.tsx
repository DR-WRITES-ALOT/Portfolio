...
const SOCIALS = [
  ...
  {
    label: "LINKEDIN",
    handle: "in/sreejith-s-h-810803243",
    ...
    <section id="signal" className="relative scroll-mt-24 pt-20 md:pt-28 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-5 grid lg:grid-cols-12 gap-12">
        <div className="lg:col-span-7">
          ...
          <Reveal delay={360}>
            <p className="mt-5 font-mono2 text-[10px] tracking-[0.2em] text-mute">
              OR PING ME ON{" "}
              <a
                href={SOCIALS[0].href}
                target="_blank"
                rel="noreferrer"
                className="text-punch font-bold hover:text-volt transition-colors underline decoration-2 underline-offset-2"
              >
                GITHUB
              </a>{" "}
              /{" "}
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-acid font-bold hover:text-volt transition-colors underline decoration-2 underline-offset-2"
              >
                LINKEDIN
              </a>{" "}
              ↗
                  ...
                  {SOCIALS.map((s) => (
                    <Magnetic key={s.label} strength={0.3}>
                      <a
                        ...
                        rel="noreferrer"
                        data-hover
                        aria-label={`Open my ${s.label} profile`}
                        className={`group flex flex-1 items-center gap-3 ${s.bg} ${s.text} border-[3px] border-ink px-4 py-3 shadow-[6px_6px_0_#05030a] transition-transform duration-150 hover:-translate-y-1`}
                      >
                        {s.icon}
                        <span className="flex flex-col">
                          ...
                          <span className={`font-mono2 text-[9px] tracking-[0.12em] mt-1 ${s.text} opacity-70`}>
                            {s.handle}
                          </span>
                        </span>
                        <span className="ml-auto font-display text-xl opacity-60 transition-transform duration-150 group-hover:translate-x-1 group-hover:-translate-y-1" aria-hidden="true">
                          ↗
                        </span>
                      </a>
                    </Magnetic>
